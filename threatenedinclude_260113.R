...
# 1. 원본 대비 Thinning 후 생존 확인
print(paste("원본 개수:", nrow(occurrence)))
print(paste("Thinning 후 개수:", nrow(clm_thinned_occ)))

# 2. PCA 투영 후 좌표값 확인
print("PCA 투영 후 PC1 범위:")
print(range(clm_occ_pca$PC1, na.rm = TRUE))

# 3. 누락된(NA) 데이터 개수 확인
missing_points <- clm_occ_pca_bins %>% filter(is.na(count_decile))
print(paste("범위를 벗어나 분류되지 않은 점 개수:", nrow(missing_points)))
